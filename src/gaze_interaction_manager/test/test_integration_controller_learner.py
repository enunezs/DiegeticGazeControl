...
from gaze_interaction_manager.msg import ButtonStatus, InteractionSegment, CalibrationModel
...
def make_button(button_id, cx, cy, t_sec, t_nanosec=0):
    msg = ButtonStatus(
        ...
        button=DiegeticButton2D(button_id=button_id, center_x=float(cx), center_y=float(cy)),
    )
...
class IntegrationSystem:
    ...
    def __init__(self):
        ...
        self.controller.corrected_gaze_pub.publish = lambda msg: self.corrected_gaze_msgs.append(msg)
...
def test_model_graduation_linear_integration():
    ...
    assert sys.controller.model_type == CalibrationModel.TYPE_LINEAR, "Controller did not receive Linear state."
...
def test_joy_reset_propagation():
    ...
    assert sys.controller.coeffs_x[5] == 0.0, "Controller bias was not reset after Joy command."
    assert sys.controller.coeffs_y[5] == 0.0, "Controller bias was not reset after Joy command."
