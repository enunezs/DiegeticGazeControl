...
def test_feature_graduation_trigger_bins():
    ...
    model = GazeCorrectionFramework(
        "TestLinear", ["bias", "lin_x"], cfg | {"trigger_bins": 3}
    )

    # Mock data arrays
    train_data = np.array([[100, 100, 5, 5], [200, 200, -5, -5], [300, 300, 2, 2]])
    ...
    assert model.current_features == [
        "bias",
        "lin_x",
    ], "Model failed to unlock features!"
...
def test_model_publication_coefficient_mapping():
    ...
    assert msg.coeffs_x[8] == 2.0 / (
        W**2
    ), "Radial X term mapped to wrong index or scaled incorrectly!"
...
def test_reservoir_spatial_block_split():
    ...
    cfg = {
        ...
        "spatial_val_ratio": 0.5,
        "max_error_cap": 200.0,
        "thinning_stride": 1,
    }
    res = SpatialReservoir(cfg)

    # Add data to two distinct bins
    # Bin (1,1)
    res.add_segment(
        np.array([150.0]), np.array([150.0]), np.array([5.0]), np.array([5.0])
    )
    # Bin (5,5)
    res.add_segment(
        np.array([550.0]), np.array([550.0]), np.array([10.0]), np.array([10.0])
    )

    train, val = res._get_spatial_block_split()

    # With ratio 0.5 and 2 bins, 1 should be val, 1 should be train
    assert len(train) > 0
    assert len(val) > 0
    # Ensure they came from different bins (checking gx values)
    assert train[0][0] != val[0][0]


# ===========================================================================
# 2. DECOUPLED FEATURE GRADUATION (NEW LOGIC)
# ===========================================================================


def test_decoupled_unlocking_policy():
    """
    Verifies that X and Y axes unlock independently under 'decoupled_shared' policy.
    """
    cfg = {
        "center_x": 800,
        "center_y": 600,
    ...
    # 1. Mock coverage: X met, Y not met
    cov = {"x": 6, "y": 2, "2d": 2}
    data = (np.array([[100, 100, 1, 1]]), np.array([[100, 100, 1, 1]]))

    model.train(data, cov, event_idx=1)

    # X should have graduated to lin_x, Y should still be bias
    assert "lin_x" in model.current_features_x
    assert model.current_features_y == ["bias"]
    assert "x_unlock" in model.unlock_moments
    assert "y_unlock" not in model.unlock_moments


def test_radial_concentric_special_mapping():
    """Radial Concentric has a hardcoded decoupled solve. Check its prediction logic."""
    cfg = {"center_x": 800, "center_y": 600}
    model = GazeCorrectionFramework(
        "Radial Concentric", ["bias", "radial_concentric"], cfg
    )

    # Manually set params: [bias, radial_coeff]
    model.params["x"] = np.array([10.0, 1.0])  # bias 10, coeff 1
    model.params["y"] = np.array([5.0, 0.5])  # bias 5, coeff 0.5

    # Gaze at (1600, 600) -> dx = 800. dx/cx = 1.0
    # px = 10 + 1.0 * (1.0) = 11.0
    px, py = model.predict(np.array([1600.0]), np.array([600.0]))

    assert px[0] == pytest.approx(11.0)
    assert py[0] == pytest.approx(5.0)
