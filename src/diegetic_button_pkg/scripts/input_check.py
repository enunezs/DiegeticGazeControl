...
cycle_duration = 0.050  # 3 seconds for complete transition #slope, we can split this later
...
class ProcessInputs(Node):  # Create node inheriting from Node
    ...
    def __init__(self):
        ...
        (self.cameraMatrix, self.distCoeffs, self.rvecs, self.tvecs) = pickle.load(
            open('src/fiducials/scripts/calibration.pckl', 'rb'))
        print('Camera intrinsics loaded!')
        # print camera intrinsics
        print(f"Camera matrix: {self.cameraMatrix}")
        print(f"Distortion coefficients: {self.distCoeffs}")
        ...
        self.frame = np.zeros([590,960, 3],dtype=np.uint8)
        self.frame.fill(0) # or img[:] = 255
        self.height, self.width = [1920/2, 1080/2]
        #TODO:
        ...
        self.subscriber_eye_info = self.create_subscription(
            String, "tobii_glasses/gaze_position", self.update_gaze_pos, 1)
        ...
        self.subscriber_front_camera = self.create_subscription(
            Image, "tobii_glasses/front_camera", self.update_frame, 1)
    ...
    def update_buttons(self, DiegeticButtonArray_msg):
        ...
        input_list = self.get_active_inputs(visible_buttons, gaze_pos, publish_image = True)
        #print inputs
        ...
        self.prev_time = self.get_clock().now()
        self.button_status = self.filter_buttons(self.button_status, input_list, delta_time, input_trigger_mode="keyboard_trigger")
        ...
        for button in self.button_status:
            ...
            if send_serial:
                message = str(button.id) + "," + \
                    str(button.status) + "," + str(button.percent)
    ...
    def get_active_inputs( self, visible_buttons, gaze_pos,  publish_image = False):
        ...
        if publish_image and self.frame is not None:
            ...
            if draw_circle:
                frame = cv2.circle(frame,
                                   (int(gaze_pos[0]*self.width),
                                    int(gaze_pos[1]*self.height)),
                                   5, (0, 0, 255), 2)
        ...
        for button in visible_buttons:
            ...
            # Get data
            rot_mat = tf.quaternion_matrix([button.button_transform.rotation.x,
            ...
            tvec = np.float32([button.button_transform.translation.x,
                               button.button_transform.translation.y,
                               button.button_transform.translation.z])
            # Plot boundary
            # +  Check overlaps
            objectPoints = np.float32([[button.x0, button.y0, 0],
                                       [button.x0, button.y1, 0],
                                       [button.x1, button.y1, 0],
                                       [button.x1, button.y0, 0]])
            img_points, _ = projectPoints(objectPoints, rvec, tvec, self.cameraMatrix,
            ...
            if draw_on_fiducials and self.frame is not None:
                ...
                drawFrameAxes(frame, self.cameraMatrix,
    ...
    def update_button_status(self, input_list, button_status_a):
        ...
        for active_input in input_list:
            ...
            if active_input not in ids:
                ...
                button = input_status(
                    active_input, status="inactive", percent=0.0)
    ...
    def filter_buttons(self, button_status, input_list, delta_time, input_trigger_mode="dwell_time"):
        ...
        if input_trigger_mode == "keyboard_trigger" or input_trigger_mode == "gamepad_trigger":
            for button in button_status:
                if button.id in input_list:
                    if self.key_pressed:
