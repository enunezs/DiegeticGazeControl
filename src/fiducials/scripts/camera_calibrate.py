# Taken from https://github.com/kyle-bersani/opencv-examples/blob/master/CalibrationByChessboard/CalibrateCamera.py
# System information:
...
objp = np.zeros(
    (CHESSBOARD_CORNERS_ROWCOUNT * CHESSBOARD_CORNERS_COLCOUNT, 3), np.float32
)
# The following line fills the tuples just generated with their values (0, 0, 0), (1, 0, 0), ...
objp[:, :2] = np.mgrid[
    0:CHESSBOARD_CORNERS_ROWCOUNT, 0:CHESSBOARD_CORNERS_COLCOUNT
].T.reshape(-1, 2)
...
for iname in images:
    ...
    board, corners = cv2.findChessboardCorners(
        gray, (CHESSBOARD_CORNERS_ROWCOUNT, CHESSBOARD_CORNERS_COLCOUNT), None
    )
    ...
    if board:
        ...
        )  # Last parameter is about termination critera
        ...
        img = cv2.drawChessboardCorners(
            img,
            (CHESSBOARD_CORNERS_ROWCOUNT, CHESSBOARD_CORNERS_COLCOUNT),
            corners_acc,
            board,
        )
...
# Make sure we wcalere able to calibrate on at least one chessboard by checking
...
# rms, K, D, rvecs, tvecs = cv2.fisheye.calibrate(    objectPoints, imagePoints, imageSize, K, D)
...
"""calibration, cameraMatrix, distCoeffs, rvecs, tvecs = cv2.fisheye.calibrate(
...
print("Camera rotation vectors:\n{}".format(rvecs))
print("Camera translation vectors:\n{}".format(tvecs))
print("Camera calibration error: {}".format(rms))


# Save values to be used where matrix+dist is required, for instance for posture estimation
# Instead of pickle, use yaml
f = open("calibration.yaml", "w")
yaml.dump((K, D, rvecs, tvecs), f)
f.close()


# Print to console our success
print(
    "Calibration successful. Calibration file used: {}".format("calibration_fish.yaml")
)
